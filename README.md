...
SQLalchemy was used to connect our jupyter notebook to pgAdmin and our PostgreSQL data base.  A connection string and engine were created to connect to our database, and export the cleaned dataframes, top_4000_movies_df and movies_df, to pgAdmin.  The dataframes were exported into our database named "movies_final" into tables called "top_4000_movies" and "movies" for the dataframes, movies_df and top_4000_movies_df respectively.
...
Before we split our features and target variable, the next step is to encode our categorical columns to numerical data to be fed into our model using sklearn label encoder library.
...
* Precision: Precision is the measure of how reliable a positive classification 
  is. From our results, the precision for the "Favorable" IMDb movie scores can be
  ...
  bad scores, 27 actually recieved a "Favorable" IMDb movie scores.
...
* Recall: Recall is the ability of the classifier to find all
  ...
  or 0.95 for the "Favorable" IMDb movie scores and 0.55 for the bad IMDB movie scores. A low
...
* In our plot we can show correlation between "Domestic_Gross" vs "score".
...
Our choice to use a random forest model has proven to be a good solution in classifying "Favorable" IMDb movie scores vs classifying "Unfavorable" IMDb movie scores. Some of the positive things about utilizing a random forest model, are that it analyzes the data by building many smaller, simpler decision trees and combining the small trees as weak learners, to create a strong learner, which has much better decision-making power. The randomly sampled data is a weaker learner when created but as the model combines those weaker learners the model can become well trained and effective at making accurate predictions. Our model was able to obtain a accuracy of 84% at identifying IMDb movie scores and classifying them. We were able to answer the question around which variables to focus on when determining IMDB movie scores. Based on the model we show that "votes" has the most impact on IMDB scores, followed by "runtime" and Domestic_Gross". So to our investors who want to know which areas to invest research in we would share those variables as the most compelling catalyst for obtaining a "Favorable" or "Unfavorable" IMDB movie score.

### Additional Links
* [Google Slides](https://docs.google.com/presentation/d/1K3iR-3VI6Z6oexiieo5eO8K5bZ4lB0vluZBnga12fdw/edit?usp=sharing)
* [Tableau Dashboard](https://public.tableau.com/app/profile/justeena5773/viz/finalproject_16425529118060/Dashboard1?publish=yes)
